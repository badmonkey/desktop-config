...
(defvar-local hl-todo--id-section-regexp nil)


(defun hl-todo--setup ()
  (setq hl-todo--id-section-regexp
        (concat "[ ]*" hl-todo-id-regexp "\\([,]?[ ]+" hl-todo-id-regexp "\\)*")
        )
  ...
  (setq hl-todo--regexp
        (concat "\\(\\(\\<"
                ...
                "\\>\\)"
                "\\([(]" hl-todo-author-regexp "[)]\\)?:"
                "\\(" hl-todo--id-section-regexp ":\\)?"
                "\\)"
        ))
